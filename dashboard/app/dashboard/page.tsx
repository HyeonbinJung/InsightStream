...
export default function DashboardPage() {
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem("insightstream:v2:snapshot");
      if (saved) {
  ...
  const filteredLogs = useMemo(() => {
    ...
    return logs
      ...
      .filter((l) => {
        ...
        if (
          q &&
          !`${l.ts} ${l.service} ${l.level} ${l.message}`.toLowerCase().includes(q)
        ) {
          return false;
        }
  ...
  function resetData() {
    ...
    localStorage.removeItem("insightstream:v2:snapshot");
    setLogs([]);
    setAlerts([]);
    setSummaries([]);
    setSelectedLog(null);
