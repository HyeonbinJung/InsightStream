...
class IncidentStore(
    ...
    private val fallbackById = Collections.synchronizedMap(LinkedHashMap<String, SavedIncident>())
    private val fallbackOrder = mutableListOf<String>()
    ...
    fun save(report: IncidentReport) {
        val vector = embedding.embed(buildEmbeddingText(report))
        val jdbc = jdbcTemplate
        ...
        afterCommit { saveFallback(report, vector) }
    }

    fun list(limit: Int = 20): List<IncidentReport> {
        val jdbc = jdbcTemplate
        if (vectorEnabled && jdbc != null) {
        ...
        return synchronized(fallbackOrder) {
            fallbackOrder.asReversed()
        ...
        if (vectorEnabled && jdbc != null) {
            val dbResult: IncidentReport? = runCatching {
                jdbc.query(
                    ...
                    ResultSetExtractor { rs -> if (rs.next()) mapReport(rs) else null },
                    id,
                )
            }.getOrElse {
    ...
    fun searchSimilar(text: String, limit: Int = 5): List<SimilarIncident> {
        val queryVector = embedding.embed(text)
        val jdbc = jdbcTemplate

        if (vectorEnabled && jdbc != null) {
            val dbResult = runCatching {
                val pgVector = embedding.toPgVector(queryVector)
                jdbc.query(
                    """
        ...
        return fallbackById.values
            .map { stored ->
                SimilarIncident(
                    ...
                    score = cosineSimilarity(queryVector, stored.embedding),
                )
            }
    ...
    private fun saveFallback(report: IncidentReport, vector: DoubleArray) {
        synchronized(fallbackOrder) {
            val existed = fallbackById.containsKey(report.id)
            fallbackById[report.id] = SavedIncident(report, vector)

            if (!existed) {
                fallbackOrder.add(report.id)
            } else {
                fallbackOrder.remove(report.id)
                fallbackOrder.add(report.id)
            }

            while (fallbackOrder.size > 500) {
                val old = fallbackOrder.removeAt(0)
